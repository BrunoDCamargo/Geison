...
Contrastive conservation prioritizes regions before oligo design. Final `specificity` remains an independent assay-level check after concrete primers and probes exist. Strong region-level contrast does not establish oligo specificity.

## Run states

The notebook uses `run_manifest.json` as the authority for run state:
...
The final section exposes the generated configs, approved panel, Geison commit, environment/tool information, checkpoint manifests, `run_manifest.json`, and raw artifact paths. Preserve these files when a run needs to be reviewed or reproduced.
...
The guided workbench supports in-silico assay discovery and evidence review. It does not replace experimental validation, wet-lab optimization, clinical validation, or the biological judgment needed to define a real diagnostic panel.
